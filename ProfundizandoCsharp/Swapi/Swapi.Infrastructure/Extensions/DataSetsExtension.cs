...
namespace SWAPI.Integracion.Infrastructure.Extensions;
        ...
        try
        {
            ...
            Parallel.ForEach(table.AsEnumerable(), row =>
            {
                ...
                Parallel.ForEach(properties, property =>
                {
                    ...
                    var notMapped = new NotMappedAttribute();
                    if (attr.Contains(notMapped)) return;
                    ...
                    if (!row.Table.Columns.Contains(propertyName)) return;
    ...
    public static T ToObject<T>(this DataRow row) where T : class, new()
    {
        ...
        try
        {
            Parallel.ForEach(properties, property =>
            {
                ...
                if (!row.Table.Columns.Contains(propertyName)) return;
                var safeValue = row[propertyName] == null ? null : Convert.ChangeType(row[propertyName], type);
                propertyInfo.SetValue(obj, safeValue, null);
            });
        }
        catch (Exception)
